...
<html lang="en">
  <head>
    <title>අපේ ICT පංතිය | Lessons</title>
    <meta charset="utf-8">
    ...
    <!--CSS for chaning banner according to mobile and desktop-->
...
<!--Destop VIEW banner-->
  ...
  <div class="hero-wrap hero-bread" style="background-image: url('images/banner3.jpg'); background-size: cover;">
      <div class="container">
        <div class="row no-gutters slider-text align-items-center justify-content-center">
          <div class="col-md-9 ftco-animate text-center">
            <br><br><p class="breadcrumbs"><span class="mr-2"><a href="index.php">No 1 </a></span> <span> ICT Class in Sri Lanka</span></p>
            <h1 class="mb-0 bread">NUWAN WIJESURENDRA</h1>
          </div>
        </div>
      </div>
...
<center>
        <div class="searchBox">
                ...
                <select id="Grade" name="Grade" id="subhome" style="padding:10px 30px; border:3px solid #2d9416;margin:2px ;">
                ...
                <button style="margin:2px; padding:10px 30px;"  type="submit" id="submitsearch" >search </button>
...
<!--**************************************************************
 ...
 <!--**************************************************************
      End of validate search function
 *****************************************************************-->
...
<!--*******************************************************
       Calculate video count for selected grade
***********************************************************-->

<?php

 $findGrade = $_GET['Grade'];


 $result1 = $conn->query("SELECT COUNT(*) AS lessonCountByGrade FROM videolessons WHERE Grade = '$findGrade'");
 ...
 ?>
 <!--*******************************************************
      End of Calculate video count for selected grade
***********************************************************-->

 <!--**************************************************************
            Show result count of selected grade
    ***************************************************************-->

    <h6><?php echo $total ?> Results For Grade <?php echo "$_GET[Grade]"; ?></h6>

    <hr style="border:1px solid green">
    
			
<!--Start of Video Lessons Grid For Selected Grade-->
              <center>
    		<div class="row">
         ...
         $findGrade = $_GET['Grade'];
				...
				include "timeago.php";
				 ...
				 $start = ($page - 1) * $limit;
				 $sql = "SELECT *FROM videolessons WHERE Grade = '$findGrade' ORDER BY lessonID DESC LIMIT  $start,$limit";
				 $result1 = $conn->query("SELECT COUNT(*) AS lessonCountByGrade FROM videolessons WHERE Grade = '$findGrade'");
				 $lessonCount = $result1->fetch_all(MYSQLI_ASSOC);
				 $total = $lessonCount[0]['lessonCountByGrade'];
...
<!--End of Video lessons Grid for selected grade-->
...
<!--Pagination-->
      <div class="row mt-5">
          <div class="col text-center">
            <div class="block-27">
              <ul>
                <li><a  href="FindVideoLessons.php?page=<?= $Previous; ?>" >&lt;</a></li>
                <?php for($i = 1; $i<= $pages; $i++) : 
                  
                  if($i == $page) { ?>
                <li class="active" ><a><?= $i; ?></a></li>
                  <?php } else { ?>
                <li ><a  href="FindVideoLessons.php?page=<?= $i; ?>&Grade=<?php echo "$findGrade" ?>"><?= $i; ?></a></li>
				        <?php } endfor; ?>
                <li><a href="FindVideoLessons.php?page=<?= $Next; ?>">&gt;</a></li>
              </ul>
            </div>
          </div>
        </div>
